/**
 * YGOProDisconnectHandler — empty-room teardown (zombie-room fix).
 *
...
const disconnect = (socketId: string, finder: YGOProRoomFinder): void => {
	new YGOProDisconnectHandler(makeSocket(socketId, true) as never, finder).run();
};

// ---------- tests ----------

describe("YGOProDisconnectHandler — empty-room teardown", () => {
	...
	it("finalizes a WAITING room when the last connected player disconnects", () => {
		...
		disconnect(lastId, { run: () => room } as unknown as YGOProRoomFinder);
	...
	it("keeps the room alive when a player disconnects but another stays connected", () => {
		...
		disconnect(leaverId, { run: () => room } as unknown as YGOProRoomFinder);
	...
	it("removes a disconnected spectator without tearing down the room", () => {
		const hostId = "sock-host";
		const host = makeClient(hostId, false);
		const room = createRoom(91004, hostId, [host]);
		const roomId = room.id;

		const spectatorId = "sock-spectator";
		const spectator = makeClient(spectatorId, true);
		const spectators = [spectator];
		Object.defineProperty(room, "spectators", { get: () => spectators, configurable: true });
		jest.spyOn(room, "spectatorLeave").mockImplementation((leaver) => {
			const index = spectators.findIndex((c) => c.socket.id === leaver.socket.id);
			if (index !== -1) {
				spectators.splice(index, 1);
			}
		});

		disconnect(spectatorId, { run: () => room } as unknown as YGOProRoomFinder);

		expect((spectator as unknown as { destroy: jest.Mock }).destroy).toHaveBeenCalledTimes(1);
		expect(spectators).toHaveLength(0);
		expect((host as unknown as { destroy: jest.Mock }).destroy).not.toHaveBeenCalled();
		expect(room.finalizing).toBe(false);
		expect(MercuryRoomList.findById(roomId)).not.toBeNull();
		expect(removeRoomBroadcasts(mockInstance.broadcast as jest.Mock)).toHaveLength(0);
	});
	...
	it("leaves no zombie room when both players disconnect one after another (real RoomFinder)", () => {
		...
		const roomId = room.id;
		const finder = new YGOProRoomFinder();
