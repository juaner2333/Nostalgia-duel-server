/**
 * YGOProDisconnectHandler — AI room teardown on human disconnect.
 *
...
const makeRoomFinder = (room: YGOProRoom): YGOProRoomFinder =>
	({ run: () => room }) as unknown as YGOProRoomFinder;

const runDisconnect = (room: YGOProRoom, humanSocketId: string): void => {
	const socket = makeSocket(humanSocketId, true);
	const handler = new YGOProDisconnectHandler(socket as never, makeRoomFinder(room));
	handler.run();
};

// ---------- tests ----------

describe("YGOProDisconnectHandler — AI room teardown", () => {
