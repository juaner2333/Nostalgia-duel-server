...
describe("YGOProRoom", () => {
	...
	// A ban-list hot-reload must NOT affect rooms already constructed. A room
	// snapshots its ban-list hash as a readonly primitive at construction, so a
	// later replaceAll() on the repository cannot mutate an in-flight room's value.
	describe("ban-list hot-reload invariant", () => {
		afterEach(() => {
			YGOProBanListMemoryRepository.replaceAll([]);
		});

		it("keeps a constructed room's banListHash after the repository is reloaded", () => {
			YGOProBanListMemoryRepository.replaceAll([createBanList("Format A", 111)]);

			const room = YGOProRoomMother.create({ command: "m#123" });
			expect(room.banListHash).toBe(111);

			// A hot-reload swaps in a same-named list with a DIFFERENT hash.
			YGOProBanListMemoryRepository.replaceAll([createBanList("Format A", 222)]);
