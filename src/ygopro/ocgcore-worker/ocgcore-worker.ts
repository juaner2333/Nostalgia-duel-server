...
export class OcgcoreWorker {
	...
	async init() {
		...
		for (let i = 0; i < 2; i++) {
			console.log(
				`[WORKER] Setting player ${i} info - LP: ${hostinfo.start_lp}, Hand: ${hostinfo.start_hand}, Draw: ${hostinfo.draw_count}`,
			);
		...
		const loadDeck = (deck: (typeof this.options.decks)[0], owner: number, player: number) => {
		...
		const loadTagDeck = (deck: (typeof this.options.decks)[0], owner: number) => {
			for (const card of [...deck.main].reverse()) {
	...
	@WorkerMethod()
	@TransportEncoder<OcgcoreProcessResultWithEncodeError, SerializableProcessResult>(
			...
			if (serialized.raw.length > 0) {
				try {
					...
					if (!messages.length) {
						...
						const consumed = messages.reduce((sum, msg) => sum + msg.toPayload().length, 0);
						if (consumed < serialized.raw.length) {
							...
							try {
								...
								trailingEncodeError = e instanceof Error ? e.message : String(e);
							}
	...
	@WorkerMethod()
	async queryFieldCount(@TransportType() query: OcgcoreQueryFieldCountParams): Promise<number> {
		return this.duel.queryFieldCount(query);
	}

	@WorkerMethod()
	@TransportEncoder<OcgcoreFieldCardQueryResult, SerializableFieldCardQueryResult>(
		// serialize in worker: only send raw
		(result) => ({
			length: result.length,
			raw: result.raw,
	...
	@WorkerMethod()
	async setRegistryValue(@TransportType() key: string, @TransportType() value: string) {
	...
	@WorkerMethod()
	async loadRegistry(@TransportType() input: Uint8Array | Record<string, string>) {
