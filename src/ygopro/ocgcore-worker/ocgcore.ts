...
const isUpdateMessage = (message: YGOProMsgBase) =>
	message instanceof YGOProMsgUpdateData || message instanceof YGOProMsgUpdateCard;
...
export class OCGCore {
	...
	private generatePlayers(seed: number[]): { name: string; deck: YGOProDeck }[] {
		...
		const shuffledDecks = this.room.shuffleDeckEnabled ? shuffleDecksBySeed(decks, seed) : decks;
	...
	async queryFieldCount({ team, location }: { team: number; location: number }): Promise<number> {
	...
	async rescheduleTimerAfterConfirm(side: number): Promise<void> {
		...
		if (elapsedMs < 10_000 && elapsedMs <= this.timerState.compensatorMs[runningPos]) {
	...
	resetResponseRequestState(): void {
		const initialTime = this.hasTimeLimit ? Math.max(0, this.room.hostInfo.time_limit) * 1000 : 0;
		this.timerState.reset(initialTime);
		this.lastResponseRequestMsg = null;
		this.isRetrying = false;
	}

	increaseResponseTime(originalDuelPos: number, gameMsg: number, response?: Buffer): void {
	...
	private async sendDeckTopMessage(client: YGOProClient, playerPosition: number): Promise<void> {
		if (!this.ocgcore) {
			return;
		}
	...
	async sendReconnectTimeLimitAndResponseState(client: YGOProClient): Promise<void> {
	...
	private async resendResponseRequestToClient(client: YGOProClient): Promise<void> {
	...
	private async sendWaitingAndTimeLimitToClient(client: YGOProClient): Promise<void> {
	...
	async routeGameMsg(
		...
		const sendToClients = options.sendToClient
			? new Set(Array.isArray(options.sendToClient) ? options.sendToClient : [options.sendToClient])
			: undefined;
	...
	private async refreshForMessage(message: YGOProMsgBase): Promise<void> {
		...
		const refreshZonePromises = zonesToRefresh.map((location) => this.refreshZone(location));
	...
	private async refreshZone(request: { player: number; location: number }): Promise<void> {
		if (!this.ocgcore) {
			return;
		}
	...
	async sendTimeLimitMessage(originalDuelPos: number, toSpecificClient?: Client): Promise<void> {
		...
		const allPlayers = [...this.room.getTeamPlayers(0), ...this.room.getTeamPlayers(1)];
		await Promise.all(
			allPlayers
				.filter((p) => !toSpecificClient || p === toSpecificClient)
				.map((client) => client.sendMessageToClient(Buffer.from(timeLimitMessage.toFullPayload()))),
		);
	}
		...
		await Promise.all(
			targetPositions.map(async (position) => {
				if (position === NetPlayerType.OBSERVER) {
					...
					await Promise.all(watchers.map((watcher) => sendGameMessage(watcher, observerView)));
				} else {
					...
					await Promise.all(
						players.map(async (client) => {
							...
							if (message instanceof YGOProMsgResponseBase && client !== actor) {
								return;
							}

							const finalMsg = client === actor ? playerView : playerView.teammateView();
	...
	private getActivePlayer(side: number): Client | null {
		const team = this.getSideTeam(side);
		const teamPlayers = this.room.players.filter((p) => p.team === team) as Client[];
		...
		const sortedPlayers = [...teamPlayers].sort((a, b) => a.position - b.position);
