...
export class MessageEmitter {
	...
	get isInvalid(): boolean {
		return this.messageProcessor.isMessageInvalid();
	}
	...
	processMessage(): void {
		// isInvalid also guards the recursion below, so a valid frame followed
		// by a zero/oversized length prefix cannot corrupt or crash the framer.
		if (this.isInvalid || !this.messageProcessor.isMessageReady()) {
			return;
		}
