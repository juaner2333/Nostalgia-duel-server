...
#### Scenario: 接受 0x1361 兼容版本首包序列
- **WHEN** TCP 或 WebSocket 连接发送版本为 `0x1361` 的有效加入序列
- **THEN** 服务器正常准入该连接，并向该客户端发送已启用 0x1361 实时对局兼容模式的系统提示，其后续接收的决斗消息帧按 0x1361 线协议自动转换

#### Scenario: 下发 MSG_CONFIRM_CARDS 消息适配
- **WHEN** 决斗核心产生 `MSG_CONFIRM_CARDS` (0x1f) 消息并下发给 0x1361 客户端
- **THEN** 服务器将 0x1362 格式中的 `skip_panel` 字节移除，并将总帧长度减 1 后下发给 0x1361 客户端

#### Scenario: 下发 MSG_SELECT_CHAIN 消息适配
- **WHEN** 决斗核心产生 `MSG_SELECT_CHAIN` (0x10) 消息并下发给 0x1361 客户端
- **THEN** 服务器将各连锁项内的 `forced` 字节移除并转换为消息头部的全局 `forced` 字节，重新打包为符合 0x1361 布局的帧后下发

#### Scenario: 下发 MSG_SELECT_SUM 消息适配
- **WHEN** 决斗核心产生 `MSG_SELECT_SUM` (0x17) 消息且候选卡片中存在包含最高位标记（0x80000000）的卡片
- **THEN** 服务器计算目标值与各操作数数值的最大公约数（GCD）进行等比缩放，保留最高符号位标记与帧长度后下发给 0x1361 客户端

#### Scenario: 混合版本房间各客户端独立适配
- **WHEN** 同一房间内同时存在 0x1361 客户端与 0x1362 客户端（包括玩家与观战者）
- **THEN** 0x1362 客户端收到未经修改的 0x1362 原始线协议帧，0x1361 客户端收到转换后的 0x1361 线协议帧，帧转换不进行缓冲区原地修改，且房间席位与生命周期一致
