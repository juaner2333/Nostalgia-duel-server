...
- 固定加入序列依次为 `ExternalAddress (0x17)`、`PlayerInfo (0x10)` 和 `JoinGame (0x12)`；协议版本基准为 `0x1362`，并直接兼容支持 `0x1361` 实时对局客户端（下发帧在客户端层按协议自动适配，内部与录像保持 0x1362）。
...
- 认证、准入和版本错误必须使用 YGOPro 原生序列化。存在兼容错误码时先发送错误再关闭；未支持版本错误（如 < 0x1361 或 > 0x1362）还必须向用户发送可读的升级客户端提示，不得以静默关闭作为唯一的用户交互；错误房间标识等静默拒绝必须保留既有关闭语义。
